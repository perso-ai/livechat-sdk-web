...
var chatbotTop = 0;
var chatbotHeight = 0;
var chatStates = new Set(); // enum PersoAILiveChat.ChatState [ RECORDING, LLM, ANALYZING, SPEAKING ], 'chatStates' can have multiple ChatState values.
...
async function getConfig() {
    ...
    try {
        config = await PersoAILiveChat.getAllSettings(apiServer, apiKey);
    } catch (e) {
        alert(e);
        return;
    };
...
async function startSession() {
    ...
    try {
        const sessionId = await PersoAILiveChat.createSessionId(
            apiServer,
            apiKey,
            llmTypeKey,
            ttsTypeKey,
        ...
        session = await PersoAILiveChat.createSession(
            apiServer, sessionId, width, height, enableVoiceChat, selectedClientTools
        );
    ...
    this.removeOnClose = session.onClose((manualClosed) => {
        if (!manualClosed) {
            setTimeout(() => {
                PersoAILiveChat.getSessionInfo(apiServer, session.getSessionId())
                    .then((response) => {
...
function processing() {
    ...
    return this.chatStates.has(PersoAILiveChat.ChatState.LLM) ||
        this.chatStates.has(PersoAILiveChat.ChatState.ANALYZING) ||
        this.chatStates.has(PersoAILiveChat.ChatState.SPEAKING);
}

function recording() {
    return this.chatStates.has(PersoAILiveChat.ChatState.RECORDING);
}
...
function applyChatStates(chatStates) {
    ...
    if (available()) {
        ...
        if (this.chatStates.has(PersoAILiveChat.ChatState.RECORDING)) {
            chatStatesTextArr.push("Recording");
        }
        if (this.chatStates.has(PersoAILiveChat.ChatState.LLM)) {
            chatStatesTextArr.push("LLM");
        }
        if (this.chatStates.has(PersoAILiveChat.ChatState.ANALYZING)) {
            chatStatesTextArr.push("Analyzing");
        }
        if (this.chatStates.has(PersoAILiveChat.ChatState.SPEAKING)) {
...
function loadChatTools() {
    ...
    const chatTool1 = new PersoAILiveChat.ChatTool(
        'get_square_number',
    ...
    const chatTool2 = new PersoAILiveChat.ChatTool(
        'get_current_weather',
    ...
    const chatTool3 = new PersoAILiveChat.ChatTool(
        'show_settings',
    ...
    const chatTool4 = new PersoAILiveChat.ChatTool(
        'show_map',
